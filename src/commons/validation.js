...
const REGEXP = {
  ...
  EMAIL: /^[a-z0-9A-Z]+([._\\-]*[a-z0-9A-Z])*@([a-z0-9A-Z]+[-a-z0-9A-Z]*[a-z0-9A-Z]+\.){1,63}[a-z0-9A-Z]{2,6}$/,
  NUMBER: /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/,
  PASSWORD: /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[~!@#$%^&*()_+`\-={}\[\]:";'<>?,.\/]).{6,16}$/,
  INTEGER: /^\d+$/,
  URL: /^([a-zA-Z]+:\/\/)?([a-zA-Z0-9\-\.]+)([-\w .\/?%&=:]*)$/,
  IDCARD: /^\d{6}(\d{4})(\d{2})(\d{2})\d{3}[0-9xX]$/i,
  CHINESE_NAME: /^[\u4e00-\u9fa5\/]{1,30}$/i,
  CHINESE: /[\u4e00-\u9fa5\/]{1,30}/i,
...
const _ = {
  ...
  isChinese: (text, isTrim) => validate(text, _.REGEXP.CHINESE, isTrim),
